...
class dispatch:
    ...
    def __call__(self, wrapped, instance, args, kwargs):
        ...
        for key in self.keys:
            data[key] = wrapped(data[key], *args, **kwargs)
        return data


def selection(key, func):
    """Apply a strategy to one section of the data."""
    def __func(data, key, func):
        data[key] = func(data[key])
        return data
    return partial(__func, key=key, func=func)
