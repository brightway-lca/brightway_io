...
class Ecospold2DataExtractor(object):
    @classmethod
    def extract_flows(cls, fp):
        def get_product_information(o):
            if not hasattr(o, "productInformation"):
                return ""
            else:
                return "; ".join(
                    [
                        ...
                        if child
                        and child.tag == "{http://www.EcoInvent.org/EcoSpold02}text"
                    ]
                )

        def extract_metadata(o):
            comment = (
                (o.comment.text if hasattr(o, "comment") else "") or ""
            ) + get_product_information(o)
            return {
                "name": o.name.text,
    ...
    @classmethod
    def extract(cls, dirpath, use_mp=True):
        dirpath = Path(dirpath)
        assert dirpath.exists() and dirpath.is_dir(), "Malformed directory path"
        assert (
            dirpath / "MasterData"
        ).is_dir(), "`dirpath` is wrong, provide parent directory with 'datasets' and 'MasterData'"
        filelist = [
            filename
            for filename in (dirpath / "datasets").iterdir()
            if filename.is_file() and filename.name.endswith(".spold")
        ]

        data = {
            "abstract flows": cls.extract_flows(
                dirpath / "MasterData" / "IntermediateExchanges.xml"
            ),
        ...
        if use_mp:
            with multiprocessing.Pool(processes=multiprocessing.cpu_count()) as pool:
                ...
                results = [
                    pool.apply_async(Ecospold2DataExtractor.extract_dataset, args=(x,))
                    for x in filelist
                ]
                data["activities and exchanges"].extend([p.get() for p in results])
        else:
            ...
            for index, filename in enumerate(filelist):
                data["activities and exchanges"].append(cls.extract_dataset(filename))
                pbar.update(item_id=filename.name[:15])
    ...
    @classmethod
    def extract_dataset(cls, filename):
    ...
    def abort_exchange(cls, exc, comment=None):
        exc["uncertainty_type_id"] = UndefinedUncertainty.id
        exc["loc"] = exc["amount"]
    ...
    def extract_uncertainty_dict(cls, obj):
        data = {"amount": float(obj.get("amount"))}
        if obj.get("formula"):
        ...
        if hasattr(obj, "uncertainty"):
            ...
            if hasattr(unc, "lognormal"):
                data.update(
                    {
                        "uncertainty_type_id": LognormalUncertainty.id,
                ...
                data.update(
                    {
                        "uncertainty_type_id": NormalUncertainty.id,
                ...
                data.update(
                    {
                        "uncertainty_type_id": TriangularUncertainty.id,
                ...
                data.update(
                    {
                        "uncertainty_type_id": UniformUncertainty.id,
                        "loc": data["amount"],
                ...
                data.update(
                    {
                        "uncertainty_type_id": UndefinedUncertainty.id,
                        "loc": data["amount"],
                    }
                )
            else:
            ...
            data.update(
                {"uncertainty_type_id": UndefinedUncertainty.id, "loc": data["amount"]}
            )
        return data
